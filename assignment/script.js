...
var addPoint_contrl=false;//Is it possible to click to added marked 
...
*/
	...
	map.on("click", "barPoints", (event) => {
		...
		*/
		function trim(x){
			const blank =/\s+/g;
			return x.replace(blank,"")
		}
//creat a function to clean blank str
		...
		const popup = new mapboxgl.Popup({
		   ...
		   <p>Url:<a href="https://${feature.properties.url.trim()}" target="_blank" > ${feature.properties.url}</a></p>`
			)
			.addTo(map);
	});
